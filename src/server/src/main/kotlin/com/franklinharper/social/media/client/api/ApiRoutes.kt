...
import com.franklinharper.social.media.client.auth.UserAlreadyExistsException
...
fun Route.registerAuthRoutes(authServiceProvider: () -> ServerSessionService) {
    route("/api/auth") {
        post("/sign-up") {
            ...
            val session = try {
                val user = authService.createUser(request.email, request.password)
                authService.createSession(user.userId)
            } catch (e: CancellationException) {
                throw e
            } catch (e: UserAlreadyExistsException) {
                call.respondText(e.message ?: "User already exists", status = HttpStatusCode.Conflict)
                return@post
            }
